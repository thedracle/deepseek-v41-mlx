"""Native DSpark (MTP) speculative decoding for DeepSeek-V4.1-Flash.
...
class DSpark(nn.Module):
    ...
    def draft_logits(self, input_token, with_confidence=False, pos_offset=0, return_hidden=False):
        ...
        mk = last.markov_head
        prev = mx.array([input_token]); toks = []; cols = []; embeds = []
        for i in range(B):
            e = mk.embed(prev); embeds.append(e)
            col = logits[:, i] + mk.head(e).astype(mx.float32)          # [1, V]
            prev = mx.argmax(col, axis=-1)
            toks.append(prev); cols.append(col)
        conf = None
        if with_confidence:
...
def load_dspark(target, model_dir, path=None):
    """Load the converted DSpark head (scripts/convert_mtp.py), by default from <model_dir>/dspark."""
    path = path or os.path.join(model_dir, "dspark")
