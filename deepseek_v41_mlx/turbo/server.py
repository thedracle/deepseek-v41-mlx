...
+ the native DSpark (MTP) speculative decoder. Single-stream, greedy, with prefix-cache reuse.
    ...
    python -m deepseek_v41_mlx.turbo.server [--port 8001] [--max-seq 131072]

Endpoints: GET /v1/models, GET /health, POST /v1/chat/completions (stream or not; tools; thinking).
Prompt format + tool-call parsing come from DeepSeek's own encoding.py (turbo/encoding.py).
...
carries reasoning_effort); V41_PATCH / V41_POST as in scripts/turbo/run.py."""
...
VP.apply(os.environ.get("V41_PATCH", VP.DEFAULT_STACK).split(","))
...
from deepseek_v41_mlx.turbo import encoding as ENC

MODEL_DIR = os.path.expanduser(os.environ.get("V41_MODEL", "~/models/DeepSeek-V4.1-Flash-MLX-mixed-4_8bit"))
...
class Engine:
    def __init__(self, max_seq):
        ...
        VP.apply_post_load(self.model, os.environ.get("V41_POST", VP.DEFAULT_POST).split(","))
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(); ap.add_argument("--port", type=int, default=8001); ap.add_argument("--max-seq", type=int, default=131072)
    a = ap.parse_args()
