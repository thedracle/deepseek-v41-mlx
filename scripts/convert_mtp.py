"""Convert the three mtp-only shards of deepseek-ai/DeepSeek-V4.1-Flash into the DSpark drafter
(<model>/dspark/dspark.safetensors + quantization.json):

    python scripts/convert_mtp.py --model ~/models/DeepSeek-V4.1-Flash-MLX-mixed-4_8bit [--src DIR]

 fp8 e4m3 (32x32 ue8m0) -> bf16, 'I8' experts (= packed fp4 e2m1 pairs,
...
import argparse, json, struct, os, re, sys, time
import numpy as np, mlx.core as mx
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."))
from deepseek_v41_mlx.dequant import dequant_fp8, dequant_fp4

MTP_SHARDS = ["model-00044-of-00048.safetensors", "model-00045-of-00048.safetensors", "model-00046-of-00048.safetensors"]
ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
ap.add_argument("--model", required=True, help="converted MLX model dir; output goes to <model>/dspark unless --out")
ap.add_argument("--src", help="dir holding the three mtp shards (default: download them from deepseek-ai/DeepSeek-V4.1-Flash)")
ap.add_argument("--out"); ap.add_argument("--bits", type=int, default=8); ap.add_argument("--group-size", type=int, default=64)
A = ap.parse_args()
SRC = A.src
if not SRC:
    from huggingface_hub import hf_hub_download
    SRC = os.path.join(A.model, "mtp-shards"); os.makedirs(SRC, exist_ok=True)
    for f in MTP_SHARDS:
        hf_hub_download("deepseek-ai/DeepSeek-V4.1-Flash", f, local_dir=SRC)
DST = A.out or os.path.join(A.model, "dspark"); os.makedirs(DST, exist_ok=True)
