...
for m, k, n in ((1, 640, 4096), (16, 640, 4096), (128, 128, 4096), (512, 128, 4096), (512, 640, 4096), (512, 640, 16384), (2048, 640, 16384), (4096, 640, 16384)):
    b, h, d = 1, 64, 512
    ...
    a = orig(q, kv, sink, idx, d ** -0.5); c = VM.sparse_attn4(q, kv, sink, idx, d ** -0.5); mx.eval(a, c)
    diff = float(mx.max(mx.abs(a.astype(mx.float32) - c.astype(mx.float32))))
    print(f"[sa4] m={m:4d} k={k:3d} n={n:5d}: port {tm(orig, q, kv, sink, idx, d**-0.5):7.2f} ms | v4 {tm(VM.sparse_attn4, q, kv, sink, idx, d**-0.5):7.2f} ms | max diff {diff:.1e}", flush=True)
